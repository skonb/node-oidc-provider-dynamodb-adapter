...
class DynamoDBAdapter {
  ...
  static isEmptyOrUndefined(MyVar) {
      return
      (
           (typeof MyVar == 'undefined') || (MyVar == null) || (MyVar == false)  //same as: !MyVariable 
               || (MyVar.length == 0) || (MyVar == "") || (MyVar.replace(/\s/g, "") == "") || (!/[^\s]/.test(MyVar)) || (/^\s*$/.test(MyVar))
      );
  }
  ...
  async upsert(id, payload, expiresIn) {
    ...
    const attrs = Object.assign(
      ...
      DynamoDBAdapter.isEmptyOrUndefined(payload) ? undefined : { payload },
